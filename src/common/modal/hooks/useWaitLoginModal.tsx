...
export function useWaitLoginModal({verifyKaKaoCode, verifyTargetEmail, onEndVerify}:{verifyTargetEmail?: string, verifyKaKaoCode?: string, onEndVerify: (authCode: string) => void}): HookMember {
    ...
    const addKakaoUser = useCallback(async (token: string) => {
    ...
    },[dispatch, findExistUserById, onEndVerify]);

    const verifyKakaoLogin = useCallback(async (code: string) => {
        if (code) {
    ...
    useEffect(() => {
        if (verifyTargetEmail !== undefined) {
            verifyEmail(verifyTargetEmail);
        }
    }, [verifyTargetEmail])

    useEffect(() => {
        if (verifyKaKaoCode !== undefined) {
            verifyKakaoLogin(verifyKaKaoCode);
        }
    }, [verifyKaKaoCode, verifyKakaoLogin])
