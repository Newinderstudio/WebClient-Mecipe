...
import {  } from '@/api/productCategoriesApi';
...
interface Props {
  onDepthCategorySelect: (categories: ProductCategoryResult[], targetId?: number) => void;
}

export interface ProductCategorySelectorHandler {
  updateCategories: (data: ProductCategoryTreeTable) => void
}

// ✅ RegionCategorySelector
const ProductCategorySelector = forwardRef<ProductCategorySelectorHandler, Props>(function ProductCategorySelector({ onDepthCategorySelect }: Props, ref) {
  const [findChildren] = useFindChildProductCategoriesByAdminMutation()

  const [selectedIds, setSelectedIds] = useState<number[]>([])
  const [categoryLevels, setCategoryLevels] = useState<ProductCategoryResult[][]>([])
  ...
  const handleSelect = async (level: number, selectedId: number) => {
    ...
    try {
      ...
      let newCategoryLevels: ProductCategoryResult[][];
